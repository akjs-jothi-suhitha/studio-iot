...
export const RealisticComponent: React.FC<RealisticComponentProps> = ({
  ...
  return (
    <g transform={`translate(${instance.x}, ${instance.y}) rotate(${instance.rotation || 0}, ${width / 2}, ${height / 2})`}>
      ...
      {/* Render interactive Pin targets */}
      {def.pins.map((pin: Pin) => {
        // Calculate pin actual absolute position relative to rotation for hover highlights
        return (
          <circle
            key={pin.id}
            cx={pin.x}
            ...
            r="4.5"
            fill="transparent"
            className="hover:fill-blue-500 hover:fill-opacity-50 cursor-crosshair stroke-none"
            data-pin-id={pin.id}
